...
<img src="doc/pictures/Keyboard.gif" width="700" >

#### 3.1.1. GUI

<img src="doc/pictures/Keyboard.png" width="700" >

| 番号 | 名前                                    | 説明                                                                                                                                                                             |
| :--- | --------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| (1)  | キーバインディングを有効にする スイッチ | 有効にするとバックグラウンドプロセスが起動してキーバインディングが利用可能になる。                                                                                               |
| (2)  | 新しいキーバインディングの追加 ボタン   | 新しいキーバインディングを追加するためのキーバインディング設定ダイアログを表示する。                                                                                             |
| (3)  | キーバインディング カード               | 設定したキーの情報を表示する。一番左には設定したキー、その右隣りは設定したアクションの情報を表示する。 <br> カード自体を選択するとキーバインディングの編集ダイアログを表示する。 |
| (4)  | キーバインディング スイッチ             | 設定したキーバインディングを有効にするかどうかを設定する。                                                                                                                       |
| (5)  | オプション                              | 各キーバインディングに対するオプションを表示する。                                                                                                                               |

---

<img src="doc/pictures/Keyboard_ThreeDot.png" width="700" >

| 番号 | 名前                    | 説明                                                                                           |
| :--- | ----------------------- | :--------------------------------------------------------------------------------------------- |
| (1)  | オプション フライアウト | 編集: キーバインディングの編集ダイアログを表示する。 <br> 削除: キーバインディングを削除する。 |

---

<img src="doc/pictures/KeyBindSettings.png" width="700" >
...
<img src="doc/pictures/KeyBindSettingsProgram.png" width="700" >

| 番号 | 名前                          | 説明                                                                                                                                                                          |
| :--- | ----------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| (1)  | アプリのパス テキストボックス | 起動するアプリを選択する。 <br> アプリを選択する ボタンを押下するとファイルピッカーを開く。 <br> ファイルが存在しているもののみ設定が可能。 <br> (拡張子まではチェックしない) |

---

<img src="doc/pictures/KeyBindSettingsUrl.png" width="700" >
...
| アクション                 | 説明                                                      | オプション                                          |
| :------------------------- | :-------------------------------------------------------- | :-------------------------------------------------- |
| アプリの起動               | オプションで設定したアプリを起動する。                    | 起動するアプリのパス                                |
...
<img src="doc/pictures/Screen.gif" width="700" >

#### 3.2.1. GUI

<img src="doc/pictures/Screen.png" width="700" >

| 番号 | 名前                        | 説明                                                                                        |
| :--- | --------------------------- | :------------------------------------------------------------------------------------------ |
| (1)  | ブルーライトカット スイッチ | ブルーライトカットの有効無効を設定する。                                                    |
| (2)  | 軽減率 スライダー           | ブルーライトカットの軽減率(%)を設定する。 <br> スライダーの設定はスロットル制限をしている。 |
...
<img src="doc/pictures/Settings.png" width="700" >
...
![クラス図](./doc/pictures/MagicSettings_Class.drawio.png)

GUI を持つアプリケーションのため、MVVM の設計で実装した。  
View 層の役割は、下記の通りにまとめた。  
また、App クラスでは DI コンテナを保持し、本プロセスのインスタンス管理と DependencyInjection の役割を集約させる。

- 設定情報の表示
- ユーザーからの入力を ViewModel 層に伝える
- 入力のバリデーションチェック

ViewModel 層の役割は、下記の通りにまとめた。  
ViewModel の変更を View 層に伝播させるため、データバインディングの仕組みを利用した。  
View のフレームワークの都合上困難なため、ViewModel はインタフェースの実装は行っていない。  
ViewModel の依存先をインタフェースにすることで単体テストを容易にした。(依存性逆転の原則)

- データの整形、保持
- データバインディングで情報の変更を通知
- 処理を Model 層に委譲する

Model 層の役割は、下記の通りにまとめた。  
Model 層は主にビジネスロジックを担当するため、インタフェースを実装し、疎結合を図った。  
Model 層に対して単体テストを実施した。
...
![クラス図](./doc/pictures/ScreenController_Class.drawio.png)

ScreenController はアプリのライフサイクルを管理する役割を担う。  
PipeServer はメインスレッドとは別のスレッドでサーバーパイプを作成して、クライアントからのメッセージを受け取れるようにする役割を担う。  
ScreenSettingsRepository は設定ファイルから設定を取得する役割と、設定を保存する役割を担う。

### 5.3. MagicSettings.KeyBindingListener.exe

多重起動はしない。  
キーイベントを監視して、設定されているキーバインディングを実行するバックグラウンドプロセス。  
他プロセスとやり取りをするため、メインスレッドとは別のスレッドでパイプを作成する。  
...
Win キー、Alt キー以外のキーダウンイベントを受け取った場合、Win キーフラグと Alt キーフラグが立っているときのみ、設定ファイル(keybindig.json)から情報を取得してキーバインディングの処理をする。
...
![クラス図](./doc/pictures/KeyBindingListener_Class.drawio.png)

本プロセスは OS からのライフサイクルイベントを受け取るためにウィンドウを作成しているが、  
ユーザーからの入力は受け付けないため、ウィンドウを非表示にしている。  
そのため、MVVM では設計していない。  
他プロセスとやり取りをするため、メインスレッドとは別のスレッドでパイプを作成する。  
パイプの処理は ServerPipe クラスが担う。  
KeyHookService クラスはキーボードイベントを受け取る役割を担い、アクションの実施は KeyboardActionService クラスが担う。  
KeyboardActionService クラスは、 KeyboardBindingRepository クラス経由で設定情報を取得し、設定に合わせて ActionFactory クラスからアクションのインスタンスを受け取り処理をする。  
MainWindow より下の層がビジネスロジックに当たるため、単体テストを実施。
...
| ファイル名     | 説明                               |
| -------------- | :--------------------------------- |
| keybindig.json | キーバインディング設定を保存する。 |
| screen.json    | スクリーン設定を保存する。         |
| theme.json     | アプリのテーマを保存する。         |
...
WinUI3 フレームワークの都合上、パイプライン上での単体テストの実施ができないため、成果物の作成のみとする。  
パイプラインは下記。  
